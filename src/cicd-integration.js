...
class WarpCICD {
  ...
  shouldTriggerTesting(url) {
    ...
    return testPatterns.some(pattern => pattern.test(url)) || this.config.environment === 'production';
  }
  ...
  generateHTMLReport(testSession) {
    ...
    const successRate = totalTests > 0 ? ((totalTests - totalFailures) / totalTests * 100).toFixed(1) : 0;
...
<body>
    <div class="container">
        ...
        <div class="suites">
            <h3>Test Suites</h3>
            ${testSession.testSuites.map(suite => `
                <div class="suite">
                    <h4>${suite.name} 
      ...
      const payload = {
        ...
        attachments: [{
          ...
          fields: [
            ...
            { title: 'Tests', value: `${this.getTotalTests(testSession)} (${this.getTotalFailures(testSession)} failed)`, short: true }
          ]
        }]
      };
      ...
      const payload = {
        ...
        "sections": [{
          ...
          "facts": [
            ...
            { "name": "Tests", "value": `${this.getTotalTests(testSession)} (${this.getTotalFailures(testSession)} failed)` }
          ]
        }]
      };
  ...
  convertToCSV(results) {
    ...
    const headers = ['Session ID', 'Environment', 'URL', 'Status', 'Total Tests', 'Failed Tests', 'Duration', 'Timestamp'];
    const rows = [headers];
...
class WarpAIClient {
  ...
  buildAnalysisPrompt(data) {
    ...
    const totalFailures = session.testSuites.reduce((total, suite) => total + (suite.failed || 0), 0);
